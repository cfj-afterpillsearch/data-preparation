...
export class PostalcodeService {
  constructor(
    ...
    @Inject(POSTALCODE_JIGYOSYO) private readonly postalcodeJigyoshoData: string[],
  ) {}
  /**
  ...
  getShikuchosonCode(_postalcode: string): string {
    // # 郵便番号を7桁の数字に整形する
    const postalcode = _postalcode.trim().replace('-', '').replace('−', '');

    const koukyoudantaiCode = 0;
    const yubinbangou7keta = 2;
    const kobetsubangou = 7;

    const res_yubin = this.postalcodeData.find((item) => item[yubinbangou7keta] === postalcode);
    if (res_yubin) {
      return res_yubin[koukyoudantaiCode];
    } else {
      const res_jigyosyo = this.postalcodeJigyoshoData.find((item_j) => item_j[kobetsubangou] === postalcode);
      if (res_jigyosyo) {
        return res_jigyosyo[koukyoudantaiCode];
      } else {
        throw new Error(`郵便番号は見つかりませんでした。, ${_postalcode}`);
      }
    }
  }
}
